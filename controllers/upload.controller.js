...
exports.uploadProfileImage = async (req, res) => {
    try {
      ...
      // Create new document
      const newDocument = {
        type: 'profilepic', // Using the correct enum value
        url: fileUrl,
        name: req.file.originalname,
        size: req.file.size,
        uploadedAt: new Date(),
...
exports.uploadResume = async (req, res) => {
  try {
    ...
    // Setup file upload to S3 or local storage
    const isS3Upload = !!req.file.location;
    ...
    // Check if the employee already has a resume and remove it
    ...
    if (existingResumeIndex !== -1) {
      ...
      if (existingResume.s3Key && isS3Upload) {
        try {
          const deleteParams = {
            Bucket: config.get("AWS_S3_BUCKET_NAME"), // Make sure this is correctly configured in your config
      ...
      // Remove the old document from the employee's array
      employee.documents.splice(existingResumeIndex, 1);
    }

    // Create the new resume document with all the required fields
    const newDocument = {
      type: 'resume',
      url: fileUrl,
      name: req.file.originalname,
      size: req.file.size,
      uploadedAt: new Date(),
      s3Key: isS3Upload ? s3Key : undefined, // Only include s3Key if it's an S3 upload
      uniqueId: uniqueKey
    };

    // Push the new document to the employee's documents array
    employee.documents.push(newDocument);

    // Save the updated employee document in the database
    await employee.save();
...
exports.uploadEducation = async (req, res) => {
    try {
      ...
      // Check if the employee already has an education document and remove it
      ...
      // Create the new education document
      const newDocument = {
        type: 'education',
        url: fileUrl,
        name: req.file.originalname,
...
exports.uploadExperienceLetter = async (req, res) => {
    try {
      ...
      // Check if the employee already has an experience letter and remove it
      ...
      // Create the new experience letter document
      const newDocument = {
  ...
  exports.uploadCertificate = async (req, res) => {
    try {
      ...
      // Check if the employee already has a certificate and remove it
